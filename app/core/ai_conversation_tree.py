...
_TREE_INIT_SYSTEM = (
    ...
    "你需要生成：\n"
    "1. 一组 /me 和 /do 文本作为「节点」（我方要发送的内容）。\n"
    "2. 2~3 条「预测路径」（对方可能的回复）。\n\n"
    "规则：\n"
    "- /me 只描述自己角色的动作、表情、语气（第一人称，不带主语）。\n"
    "- /do 只描述环境、可观察状态、声音与客观信息。\n"
    "- 预测路径应是对方角色可能发出的 /me 或 /do，覆盖不同可能性（如配合、抗拒、中立）。\n"
    "- 每条文本不超过80个字符。\n\n"
    "输出格式（必须严格遵守，只输出JSON，不要其他文字）：\n"
    '{"node":[{"type":"me","content":"..."},{"type":"do","content":"..."}],'
...
_TREE_NEXT_SYSTEM = (
    ...
    "生成下一轮我方「节点」（要发送的 /me /do 文本）和新的「预测路径」（对方可能的回复）。\n\n"
    "规则：\n"
    "- /me 只描述自己角色的动作、表情、语气（第一人称，不带主语）。\n"
    "- /do 只描述环境、可观察状态、声音与客观信息。\n"
    "- 严禁替他人写动作、心理、决定。\n"
...
_TREE_WRAPUP_SYSTEM = (
    ...
    "生成一组能够**优雅地收尾并结束当前场景**的 /me 和 /do 文本。\n\n"
    "规则：\n"
    "- 收尾节点应给场景画上自然的句号，让对方也能顺势结束。\n"
    "- 可以包含告别、离开、总结性动作等。\n"
    "- /me 只描述自己角色的动作。\n"
    "- /do 只描述环境变化和客观状态。\n"
    "- 严禁替他人行动。\n"
